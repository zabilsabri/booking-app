...
@section('content')
    ...
    <div class="container-dropdown-schedule align-content-center">
        <form action="{{ route('book-process') }}" method="post">
            @csrf
            <div class="dropdown-schedule d-flex align-content-center w-100 border">
                ...
                <div class="p-2 flex-fill">
                    <input type="date" name="book_date" class="form-control" min="{{ date('Y-m-d') }}" required>
                </div>
                <div class="p-2 flex-fill">
                    <input class="form-control" type="text" name="sessions" placeholder="Masukkan Jumlah Sesi" aria-label="sesi_input" required>
                </div>
